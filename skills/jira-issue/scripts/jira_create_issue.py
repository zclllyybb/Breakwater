...
@click.option("--epic", help="Epic issue key to link this issue to (e.g., OPS-1000)")
...
def main(
    ...
    if assignee and "@" in assignee:
        click.echo(
            "Warning: assignee expects Jira username, not email (e.g. alice).",
            err=True,
        )
    ...
    if key and epic:
        ...
        epic_resp = requests.post(epic_url, headers=headers, auth=request_auth, json=epic_payload, timeout=20)
        if epic_resp.status_code >= 400:
            click.echo(f"Warning: Failed to link issue to epic: {epic_resp.status_code}", err=True)
