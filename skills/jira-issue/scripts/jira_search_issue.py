...
@click.option("--output-file", type=click.Path(dir_okay=False, path_type=Path), help="Write full JSON response to a file and print only a small summary")
...
def main(
    ...
    if output_file is not None:
        ...
        keys = [str(issue.get("key")) for issue in issues if issue.get("key")]
        click.echo(f"total={total}")
        click.echo(f"returned={len(issues)}")
        click.echo(f"output_file={output_file}")
        click.echo(f"output_bytes={output_file.stat().st_size}")
        if keys:
            suffix = ",..." if len(keys) > 20 else ""
            click.echo(f"keys={','.join(keys[:20])}{suffix}")
        if next_token:
